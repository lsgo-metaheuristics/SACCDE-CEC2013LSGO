...
RBFNetwork::RBFNetwork(const vector<datapoint> &training_data, const vector<double> &training_values)
...
RBFNetwork::RBFNetwork(const vector<datapoint> &training_data, const vector<double> &training_values,
                       const vector<datapoint> &testing_data, const vector<double> &testing_values)
...
double RBFNetwork::training(int num_rbf_units, double learning_rate, int num_iterations, double toll, bool print_flag)
{
...
for (auto &w : layer2_weights)
    ...
    double mse = 0, prev_testing_mse = 1.0E100;
    for (int iter = 0; iter < num_iterations; iter++)
    {
        ...
        for (int jj = 0; jj < training_data.size(); jj++)
        {
            ...
            double error_j = training_values[j] - Fj;
        ...
        if (testing_data.size()>0)
        {
            ...
            {
                double error = testing_values[i] - predictValue(testing_data[i]);
        ...
        if (print_flag)
        {
            ...
            for (int i = 0; i<training_data.size(); i++)
            {
                double error = training_values[i] - predictValue(training_data[i]);
                mse += error * error;
            }
...
void RBFNetwork::build_hxc()
{
    ...
    // Build RBF Units
    hxc.assign(training_data.size(),vector<double>());
    for(int i = 0 ; i<training_data.size() ; i++)
    {
...
double RBFNetwork::basisFunction(const datapoint &data_point, const int centroidIndex)
{
...
double RBFNetwork::predictValue(const datapoint &data_point)
{
    vector<double> cur_rbf_unit;
    for(int j = 0 ; j<rbf_centroids.size() ; j++)
        cur_rbf_unit.push_back(basisFunction(data_point, j));
    double v = Utility::multiplyVectors(cur_rbf_unit, layer2_weights);
    return v;
}
...
void RBFNetwork::testing(const std::vector<datapoint> &testing_data, const std::vector<double> &testing_values)
{
    printf("Testing...\n");
    double mse = 0, err = 0;
    for(int i = 0 ; i<testing_data.size() ; i++)
