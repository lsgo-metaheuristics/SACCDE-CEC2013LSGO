...
public:
	RBFNetwork();
	RBFNetwork(const std::vector<datapoint> &training_data, const std::vector<double> &training_values);
	RBFNetwork(const std::vector<datapoint> &training_data, const std::vector<double> &training_values,
		       const std::vector<datapoint> &testing_data, const std::vector<double> &testing_values);
	~RBFNetwork(void);
	...
	/* Start Training the Radial Basis Function network
		...
		*/
	double training(int num_rbf_units, double learning_rate, int num_iterations, double toll, bool print_flag = false);
	//double training(int num_rbf_units, double learning_rate, int num_iterations, double toll, bool print_flag = false);

	/* Start Testing the RBF Network to make sure it's not overfitting
		(Should be done after training) */
	void testing(const std::vector<datapoint> &testing_data, const std::vector<double> &testing_values);

	
	/* Predict a single data point*/
	double predictValue(const datapoint &data_point);
...
private:	
	...
	std::vector< std::vector<double> > hxc;
	...
	double basisFunction(const datapoint &data_point, const int centroidIndex);
